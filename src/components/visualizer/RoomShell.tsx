...
export function RoomShell({ width, length, height, showCeiling, onPlace }: RoomShellProps) {
  return (
    <group>
      ...
      <Wall id="back" position={[0, height / 2, -length / 2]} size={[width, height, 0.1]} onPlace={onPlace} />
      <Wall id="left" position={[-width / 2, height / 2, 0]} size={[0.1, height, length]} onPlace={onPlace} />
      <Wall id="right" position={[width / 2, height / 2, 0]} size={[0.1, height, length]} onPlace={onPlace} />
